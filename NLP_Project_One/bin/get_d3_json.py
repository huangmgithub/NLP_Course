...
def save_json():
    ...
    with open(os.path.join(BASE_DIR, 'result.txt'),  encoding='utf-8') as f, \
        ...
        for line in f:
            ...
            # nodes
            if {"id":l[0], "group":1} not in res["nodes"]:
                res["nodes"].append({"id":l[0], "group":1})
            if {"id": l[1], "group":2} not in res["nodes"]:
                res["nodes"].append({"id":l[1], "group":2})
            if {"id": l[2], "group":3} not in res["nodes"]:
                res["nodes"].append({"id":l[2], "group":3})
            # links
